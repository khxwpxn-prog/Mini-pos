...
export default function SellPage() {
  ...
  // ฟังก์ชันยิงผ่าน API Route ฝั่ง Server (ยิงไปที่ /api/telegram)
  const sendTelegramNotification = async (message) => {
    try {
      ...
      if (!resData.ok) {
        alert(`❌ Telegram Error: ${resData.description || 'ส่งข้อความไม่สำเร็จ'}`);
      }
    } catch (err) {
      console.error('ส่งข้อความ Telegram ไม่สำเร็จ:', err);
      alert('❌ เกิดข้อผิดพลาดในการเชื่อมต่อระบบแจ้งเตือน');
    }
  };
